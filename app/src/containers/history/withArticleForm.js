  ...
  return class extends React.Component {
    ...
    processForm(event) {
      ...
      if (validatedForm.isValid) {
        apiArticleHistory[apiMethod](validatedForm.payload).then((review) => {
          // Dispatch action to add review to history
          this.props[apiMethod](review);
          this.setState({ errors: {} });
          this.props.history.push('/history/articles');
        })
        .catch(error => {
          if (error.code === 401) {
    ...
    validateForm() {
      ...
      // Create a article out of the state properties (minus the errors) which could then
      ...
      if (!isEmpty(payload.link) && !isURL(payload.link)) {
        errors.link = 'Please provide a valid URL for the article';
      }
...
export default withArticleForm;
